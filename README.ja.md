...
> **v0.3.0 は本番接続前のローカル統合版です。** `watch` によるCSV/JSONL書き出しの継続監視と、二者承認後に別プロセスの防御側へ署名付き対応要求を送る経路を実装しています。VPN・IdPの資格情報は保持せず、実環境のセッション失効や通信遮断は接続先のレスポンダーが検証して実行します。標準動作は決定論的ルールとテンプレート説明であり、生成AIは使用しません。
>
