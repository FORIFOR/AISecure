...
> **v0.2.0 は製品化前のローカル検証版です。** 常時監視、実ログの自動収集、VPN保護、実環境のセッション失効や通信遮断は行いません。標準動作は決定論的ルールとテンプレート説明であり、生成AIは使用しません。任意のローカルOllama説明アダプターは実装済みですが、今回の環境では実モデル接続・出力品質を未検証です。
>
