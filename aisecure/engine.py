...
def analyze(snapshot: dict, config: RuleConfig | None = None) -> list[dict]:
    ...
    for aid, a in assets.items():
        ...
        if a["patch_state"] == "pending":
            ...
            result.append(finding("AS-001", p, "公開機器の修正を優先" if exposed else "未適用パッチの確認", reasons, ["asset:" + aid], config, asset_id=aid, kind="exposure", cvss=v["cvss"], reference=v["reference"]))
        ...
        if unknown or stale:
            result.append(finding("AS-005", "P2", "資産情報の不足・古さを確認", [f"未確認の項目や{config.stale_asset_hours}時間を超える古い資産情報を、安全と扱いません。"], ["asset:" + aid], config, kind="coverage", asset_id=aid))
    ...
    for e in events:
        ...
        if suspicious_login(e, config):
            result.append(finding("AS-002", "P2", "特権ログインの条件を確認", [("特権ログインに、未承認作業と非管理端末の条件が重なっています。" if config.identity_conditions == "both" else "特権ログインに、未承認作業または非管理端末の条件が重なっています。"), "不正利用と断定せず、作業申請と端末状態を確認してください。"], [e["id"]], config, kind="identity", actor=e["actor"], gateway_id=e["gateway_id"], at=e["at"]))
    ...
    for (actor, session, target), reads in groups.items():
        ...
        result.append(finding("AS-003", "P2", "短時間の大量ファイル参照", [f"{config.window_seconds}秒以内に{peak}個の異なるファイルへの参照を記録しました。", f"このうち機密ラベル付きは{sensitive}個です。", "参照ログだけでは外部への送信・漏えいを証明できません。"], evidence, config, kind="bulk", actor=actor, asset_id=target, distinct_files=peak, sensitive_files=sensitive, at=best[-1]["at"]))
