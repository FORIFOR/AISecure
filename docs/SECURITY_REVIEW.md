...
### 問題11: 未来日時の1行で取り込み全体が失敗（重要度: 低 / 修正済み・ファズで発見）

取り込み境界に**ランダム/敵対的入力を大量に流すファズテスト**（`tests/test_fuzz.py`）を追加したところ、巨大なepoch値など**取り込み時刻より未来の日時を持つイベントが1行あると、スナップショットの `as_of` がその未来時刻になり、後段の `normalize()` が「未来すぎる」として全体を拒否**する経路を発見しました（「1行の異常で全体が落ちない」原則の抜け）。取り込み時に「現在＋5分」より未来のイベント・資産行を読み飛ばして計上・警告するようにし、1件のみ未来日時があっても残りは正常に取り込めることを確認しました。

### 問題10: 静的プレビュー(try.html)のJSON埋め込み（重要度: 低 / 予防的に修正）

`tools/make-preview.py` と `tools/capture.py` は、合成スナップショットを `<script>` 内に `json.dumps` でそのまま埋め込んでいました。**現状の合成データには `<`・`</script>`・U+2028/2029 が含まれないため実害はありません**でしたが、将来データが変われば `</script>` でブレイクアウトし得る潜在的XSSです。埋め込み時に `<`→`\u003c` 等へエスケープする安全策を入れ、`</script><script>alert(1)</script>` を無害化することを確認しました。**実際に動くアプリ(server.py)はHTMLにstateを埋め込まず `/api/state` でJSON配信するため、この経路の影響は受けません。**
...
| 試したこと | 結果 |
|---|---|
| 10万段ネストのJSON、400 KBの1行、バイナリ列、巨大数、**ファズ620ケース**（ランダムCSV/JSONL/バイナリ） | 行単位で読み飛ばし、未処理例外なし（**ただしPython 3.11では当初落ちていた。問題9**） |
...
| **HTTP境界のファズ200件**（不正メソッド・パス・ヘッダ・本文をランダム生成） | クラッシュ・ハングなし、内部パス/トレースバック非漏洩、認証は常に強制、事後も正常応答 |
