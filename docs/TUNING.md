...
ここを通過してから、承認付きの実対応へ進む構成です。v0.3.0では、Oktaユーザーのセッション失効に限り、`execute-okta` が明示された対象IDを使って実行し、`user.session.clear` の確認後だけ成功と記録します。実組織での権限・業務影響・復旧手順は導入先で検証してください。
